...
@implementation BWMapHelper

@synthesize pins;

- (id)init {
    if ((self = [super init])) {
        mapView = [(BWMainViewController *)[[APPDELEGATE window] rootViewController] mapView];
...
- (void)requestDirectionsFrom:(MKPlacemark *)src to:(MKPlacemark *)dest withWaypoints:(NSArray *)wpList withCompletionHandler:(void (^)(NSArray *, NSError *))completionHandler {
    ...
    BWVenue *start = [wpList objectAtIndex:0];
    ...
    for (int i = 1; i < wpList.count; i++) {
        BWVenue *venue = [wpList objectAtIndex:i];
    ...
    NSDictionary *dict = [NSJSONSerialization  JSONObjectWithData:[routeString dataUsingEncoding:NSUTF8StringEncoding]
                                                          options:0
                                                            error:nil];
