...
- (void)viewDidLoad
{
    ...
    initialViewController = (InitialViewController *)[[APPDELEGATE window] rootViewController];
...
- (void)routeBywayWithVenues:(NSArray *)venueList {
    if (venueList) {
        ...
        for (int i = 0; i < venueList.count; i++) {
            Venue *venue = [venueList objectAtIndex:i];
...
- (void)trimToBestVenues {
    ...
    for (NSString *key in venuesDict) {
        Venue *venue = [venuesDict objectForKey:key];
    ...
    if (venues.count > idealCount) {
        [venues sortUsingComparator:^NSComparisonResult(Venue *obj1, Venue *obj2) {
